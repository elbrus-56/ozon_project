...
class NotifyClientArbitration:
    
    def __init__(self):
        self.om = OzonManager()
        self.tusmk = TusMK()
    
    def notify_client_arbitration(self, email=False, telegram=False):
        # initialize Ozon and TUS MK interfaces
        
        # get new (awaiting_packaging) orders
        new_orders = self.om.get_client_arbitration_orders()
            ...
            for order in new_orders["result"]["postings"]:
                ...
                if len(exist) > 0:
                    ...
                    if order["status"] != exist[0].status:
                        ...
                        exist[0].status = "client_arbitration"
                        exist[0].save()
